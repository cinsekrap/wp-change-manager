...
<x-admin.page-header
    ...
    lede="Someone who can agree to spend content design hours." />

<form method="POST" action="{{ $approver->exists ? route('admin.funding-approvers.update', $approver) : route('admin.funding-approvers.store') }}" class="card max-w-3xl">
    @csrf
    ...
    <x-admin.form-section title="Who they are" help="Their name and role appear on the record of what they signed off.">
        ...
        <div class="field mb-0">
            ...
            <input type="text" name="job_title" id="job_title" value="{{ old('job_title', $approver->job_title) }}"
                placeholder="e.g. Head of Communications" class="field-input">
            @error('job_title') <p class="field-error">{{ $message }}</p> @enderror
        </div>
    </x-admin.form-section>

    <x-admin.form-section title="Their remit" help="Shown when someone picks an approver, so they can tell this is the right person to ask.">
        <div class="field mb-0">
            <label for="remit" class="field-label">What budget they hold</label>
            <textarea name="remit" id="remit" rows="3" placeholder="e.g. Community services content budget, BSW and Bath"
    ...
    <x-admin.form-actions>
        ...
        <a href="{{ route('admin.funding-approvers.index') }}" class="btn btn-quiet">Cancel</a>
    </x-admin.form-actions>
</form>
@endsection
