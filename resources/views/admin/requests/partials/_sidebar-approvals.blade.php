...
<div class="bg-white rounded-lg shadow">
    <button type="button" onclick="this.nextElementSibling.classList.toggle('hidden'); this.querySelector('.chevron').classList.toggle('rotate-180')"
        ...
        <div class="flex items-center space-x-2">
            ...
            @if($changeRequest->approval_overridden)
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-700">Overridden</span>
            @elseif($changeRequest->approvers->isNotEmpty())
                ...
                @endif
            @else
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-500">None</span>
            @endif
        </div>
    ...
    <div class="hidden border-t border-gray-100 p-4">
        ...
        @php
            $grouped = $changeRequest->approvers->whereNotNull('group')->groupBy('group');
            $ungrouped = $changeRequest->approvers->whereNull('group');
        @endphp
        <div class="space-y-3 mb-4">
            {{-- Grouped approvers --}}
            @foreach($grouped as $groupName => $members)
            @php
                $groupApproved = $members->contains(fn($a) => $a->status === 'approved');
                $groupRejected = $members->contains(fn($a) => $a->status === 'rejected');
            @endphp
            <div class="border border-gray-200 rounded-lg overflow-hidden">
                <div class="flex items-center justify-between px-3 py-2 bg-gray-50">
                    <span class="text-xs font-semibold text-gray-600">{{ $groupName }}</span>
                    @if($groupApproved)
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">Satisfied</span>
                    @elseif($groupRejected)
                ...
                <div class="divide-y divide-gray-100">
                    ...
                    <div class="px-3 py-2">
                        ...
                        @if($approver->responded_at)
                            <p class="text-xs text-gray-400 mt-0.5">{{ $approver->responded_at->format('d M Y H:i') }} ({{ $approver->responded_at->diffForHumans() }})</p>
                        @endif
                        ...
                        @if($approver->isPending() && !$groupApproved)
